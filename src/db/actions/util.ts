...
export async function checkFamilyMembershipRole(familyId: Family['id'],
                                                roleType: Membership['roleType'],
                                                checkDescending: boolean = false): Promise<boolean> {
  const userId = await getAuthorId();
  ...
  switch (membership.roleType) {
    case RoleType.ADMIN: return true; // ADMIN has all the rights
    case RoleType.MODERATOR: return roleType === RoleType.MODERATOR || roleType === RoleType.USER; // MODERATOR is USER too
    case RoleType.USER: return roleType === RoleType.USER; // USER means USER, lowest role
    default: return exhaustiveGuardRoleType(membership.roleType);
  }
}